...
Analýza četnosti návštěv je střednědobá technika\footnote{Střednědobá technika znamená, že její zavedení vyžaduje nějaký časový rámec, například 2 týdny.}, která slouží k rozpoznávání automatického přístupu na základě časů přihlášení~--~intervaly mezi přihlášeními, časy přihlášení apod. Pro analýzu četnosti návštěv je třeba nejprve získat data o~uživatelských návštěvách sociální sítě, následně je analyzovat a~až poté na jejich základě definovat opatření. Doba, po kterou se získávají vědomosti o~uživatelském provozu, může být různá, ale obecně platí, že dlouhodobá analýza je nejpřesnější. Počet návštěv v~pracovní den se může lišit od počtu návštěv v~nepracovní den či může záviset na denním čase (např.~restaurace budou mít vyšší provoz v~čase oběda). Existuje více metod, jak detekovat podezřelý provoz touto analýzou, každá ale závisí na typu webové služby. V~případě sociálních sítí může být považována za podezřelou ta aktivita, při které dochází v~krátkých časových intervalech ke~snaze o~přihlášení se na jeden účet či pravidelné přihlašování se ve stejný čas.
...
Většina automatických botů, které se snaží o~získání dat z~webové stránky, má přesně danou pozici a~dokonale zná HTML (viz~\ref{sec:html}) strukturu navštívené stránky. Na základě této znalosti dokážou automatičtí boti získávat požadovaná data. Při změně této struktury (např. hlubší zanoření HTML elementů) může dojít k neschopnosti automatického robota získat požadovaná data.

\subsection*{Změna identifikátorů značek HTML}
...
\begin{itemize}
  \item Automatický test stejného chování (automaticky pomocí Selenium WebDriver)
  \item Automatický test hromadného přístupu (automaticky pomocí Selenium WebDriver)
  \item Test ověřování \texttt{User-Agent} hodnoty (manuálně pomocí Selenium WebDriver)
  \item Přístup z~prohlížeče Tor (manuálně pomocí prohlížeče Tor)
  \item Test kódování identifikátorů HTML elementů (manuálně pomocí prohlížeče Google Chrome)
\end{itemize}
...
Každá načítaná stránka ve webovém prohlížeči se načítá pomocí HTML jazyka (viz~\ref{sec:html}). Webscrapingové automaty často pracují právě s~HTML strukturou a~jednotlivé elementy poznávají podle jedinečných HTML identifikátorů k~získání dat, které tento HTML element obsahuje. V~testu je ověřeno, zda webová stránka používá intuitivní názvy HTML identifikátorů (např. \texttt{heading}) nebo zda používá nějaké kódování (např. \texttt{0h4jek3iS}). Testy změny jmen identifikátorů byly ověřeny ručně pomocí nástroje Google Developers (více zde~\ref{sec:chrome_devtools}).
...
\section{Návrh aplikačního firewallu}
Tato sekce obsahuje podrobný popis jednotlivých opatření, které jsou implementovány v~rámci aplikačního firewallu Socnetu. Návrh řešení spočívá \textit{zejména} v~odhalování stejného uživatelského chování. Tento postup byl navrhnut z~důvodu, že žádná sociální síť neodhalila naprosto identické chování $50\times$ za sebou v~jedné hodině, během které se navíc uživatel $50\times$ přihlásit. Ostatní navrhované testy také vychází z~analýzy aplikačních firewallů reálných sociálních sítí, protože většina firewallů sociálních sítí neodhaluje takový provoz, který je velmi podezřelý a~téměř jistě není v~souladu s~normálním lidským chováním.
...
\noindent
Pokud se uživatel v~době své blokace pokusí přihlásit znovu, doba k jeho nejbližšímu přihlášení se aktualizuje na náhodnou dobu v~intervalu <10,30>~sekund.
